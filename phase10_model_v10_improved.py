...
time_fold_indices = list(skf.split(train, train["is_mule"]))

print("✅ StratifiedKFold folds built")
...
print("Optuna disabled — using pre-tuned defaults")
...
n_contributed = 0  # track folds that actually trained on both classes
...
for fold_i, (tr_idx, val_idx) in enumerate(time_folds_clean):
    ...
    # Skip fold if training set has only one class (LabelEncoder would fail on eval_set)
    if len(np.unique(ytr)) < 2:
        print(f"  Skipping fold {fold_i+1}: training set contains only class "
              f"{np.unique(ytr)[0]} — insufficient for fitting")
        continue
    ...
    t_xgb += m2.predict_proba(X_test)[:, 1]
    if len(np.unique(yval)) > 1:
        print(f"  XGB  AUC={roc_auc_score(yval, oof_xgb[val_idx]):.4f}")
    else:
        print(f"  XGB  AUC=N/A (single-class val)")

    # CatBoost
    m3 = CatBoostClassifier(**best_cat_params)
    m3.fit(Xtr, ytr, eval_set=(Xval, yval))
    oof_cat[val_idx] = m3.predict_proba(Xval)[:, 1]
    t_cat += m3.predict_proba(X_test)[:, 1]
    if len(np.unique(yval)) > 1:
        print(f"  CAT  AUC={roc_auc_score(yval, oof_cat[val_idx]):.4f}")
    else:
        print(f"  CAT  AUC=N/A (single-class val)")

# Average test predictions over contributed folds
if n_contributed > 0:
    t_lgb /= n_contributed
    t_xgb /= n_contributed
    t_cat /= n_contributed
    print(f"\nContributed folds: {n_contributed}/{n_active_folds}")
else:
    print("WARNING: no folds contributed — all training sets were single-class")
...
if analysis_cols:
    fp_df = train_clean[fp_mask][analysis_cols].describe()
    fn_df = train_clean[fn_mask][analysis_cols].describe()
    tp_df = train_clean[(oof_binary == 1) & (y_clean == 1)][analysis_cols].describe()
...
if err_new_features:
    ...
    for tr_idx, val_idx in time_folds_clean:
        ytr_v2 = y_clean[tr_idx]
        if len(np.unique(ytr_v2)) < 2:
            continue
        n_contributed_v2 += 1
        m_v2 = lgb.LGBMClassifier(**best_lgb_params)
        m_v2.fit(X_clean_v2[tr_idx], ytr_v2,
                 sample_weight=sample_weights[tr_idx],
                 eval_set=[(X_clean_v2[val_idx], y_clean[val_idx])],
                 callbacks=[lgb.early_stopping(80, verbose=False),
                             lgb.log_evaluation(period=-1)])
        oof_lgb_v2[val_idx] = m_v2.predict_proba(X_clean_v2[val_idx])[:, 1]
        t_lgb_v2 += m_v2.predict_proba(X_test_v2)[:, 1]
    if n_contributed_v2 > 0:
        t_lgb_v2 /= n_contributed_v2
